...
1. Create an issue for the blog post. This is to help the editors track the progress of the post.
   ...
   * **Release blog posts** (Open Liberty release announcements only)
     ...
     Create [an Open Liberty release notes issue](https://github.com/OpenLiberty/blogs/issues/new?assignees=lauracowen&labels=release&template=release-blog-post.md&title=Release+notes+for+VERSION_NUMBER). Make sure to select each task in the issue as you complete it to show progress.
